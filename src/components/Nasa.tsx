...
const Nasa = forwardRef((props, newsRef: any) => {
  ...
  // Curiosity rover state
  const [rover, setRover] = useState<any>(null);

  useEffect(() => {
    const fetchRover = async () => {
      const roverApi =
        "https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=1000&camera=mast&api_key=DrZ0rvpXxkznekZtUtea1GbO2e6chwGx6aC0h6wN";
    ...
    const fetchArticle = async () => {
      const newsApi =
        "https://api.nasa.gov/planetary/apod?api_key=DrZ0rvpXxkznekZtUtea1GbO2e6chwGx6aC0h6wN";
      ...
      try {
        ...
        console.log(
          "Article data for today isn't available yet, displaying article data from yesterday temporarily till it becomes available."
        );
  ...
  function getDivImg() {
    let image_url =
      rover.photos[Math.floor(Math.random() * rover.photos.length)].img_src;
    const ret = {
                ...
                {article && (
                  <>
                    ...
                    <img
                      ...
                      alt="article1"
                    />
                    <p className="text-grey-darker text-base text-justify mt-4 my-4">
                      {article.explanation}
                    </p>
